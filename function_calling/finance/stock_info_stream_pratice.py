...
def tool_list_to_tool_obj(tools):
    ...
    tool_calls_dict = defaultdict(
        lambda: {"id": None, "function": {"name": None, "arguments": ""}, "type": None}
    )

    for tool_call in tools:
        print(f"tool_call_!: {tool_call}")
...
if user_input := st.chat_input():  # ① 사용자 입력 받기
    ...
    with st.chat_message(
        "assistant"
    ).empty():  # 블록안에 chunk단위로 오는 메시지를 계속해서 출력

        for chunk in ai_response:
            print(f"chunk 호출 발생생: {chunk}")
            ...
            print(f"content_chunk_type: {type(tool_calls_chunk)}")
